...
with open('/home/ubuntu/hydrafloodstool/hydrafloodviewer/static/data/laos/laos_flooded_districts.txt', "a+") as f:
...
with open('/home/ubuntu/hydrafloodstool/hydrafloodviewer/static/data/laos/laos_flooded_districts.txt', "a+") as f:
  ...
  if len(data) <= 0:
    ...
    # Select Laos
    laos = countries.filter(ee.Filter.eq("ADM0_NAME", "Laos"))

    # Clip flood layer to laos 
    floodedWaterLaos = floodedWater.clip(laos)
    selectLaosFloodedWaterPixel = floodedWaterLaos.multiply(ee.Image.pixelArea()).divide(1e6).set('date', ee.Date(floodedWaterLaos.get('system:time_start')).format('YYYY-MM-dd'))
    #print(selectLaosFloodedWaterPixel.getInfo())

    DISTRICT_FLOOD_THRESHOLD = 10

    def calcArea(feature):
        fldArea = selectLaosFloodedWaterPixel.unmask(0).reduceRegion(
    ...
    districtArea = laos.map(calcArea)
    #print(districtArea)
    laosFloodedDistricts = districtArea.filter(ee.Filter.gt("water",DISTRICT_FLOOD_THRESHOLD))
    ...
    print(laosFloodedAreaDate.getInfo(), laosFloodedDistricts.size().getInfo())

    f.write("\n")
    # Append text at the end of file
    output = str(laosFloodedAreaDate.getInfo())+','+ str(laosFloodedDistricts.size().getInfo())
    f.write(output)
  else: 
    # Get last row 

...
with open('/home/ubuntu/hydrafloodstool/hydrafloodviewer/static/data/vietnam/vietnam_flooded_districts.txt', "a+") as f:
...
with open('/home/ubuntu/hydrafloodstool/hydrafloodviewer/static/data/vietnam/vietnam_flooded_districts.txt', "a+") as f:
  ...
  if len(data) <= 0:
    ...
    selectVietnamFloodedWaterPixel = floodedWaterVietnam.multiply(ee.Image.pixelArea()).divide(1e6).set('date', ee.Date(floodedWaterVietnam.get('system:time_start')).format('YYYY-MM-dd'))
    #print(selectVietnamFloodedWaterPixel.getInfo())

    DISTRICT_FLOOD_THRESHOLD = 10

    def calcArea(feature):
        fldArea = selectVietnamFloodedWaterPixel.unmask(0).reduceRegion(
    ...
    districtArea = vietnam.map(calcArea)
    #print(districtArea)
    vietnamFloodedDistricts = districtArea.filter(ee.Filter.gt("water",DISTRICT_FLOOD_THRESHOLD))
    ...
    print(vietnamFloodedAreaDate.getInfo(), vietnamFloodedDistricts.size().getInfo())

    f.write("\n")
    # Append text at the end of file
    output = str(vietnamFloodedAreaDate.getInfo())+','+ str(vietnamFloodedDistricts.size().getInfo())
    f.write(output)
  else: 
    # Get last row 
